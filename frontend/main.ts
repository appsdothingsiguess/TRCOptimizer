...
document.addEventListener("DOMContentLoaded", () => {
  const assetTagEl = document.getElementById("asset-tag") as HTMLInputElement;
  const iiqTicketEl = document.getElementById("iiq-ticket") as HTMLInputElement;
  const techInitialsEl = document.getElementById("tech-initials") as HTMLInputElement;
  const runIntakeBtn = document.getElementById("run-intake") as HTMLButtonElement;
  ...
  function setStatus(text: string, cls: string | null): void {
  ...
  ws.onmessage = (event: MessageEvent) => {
    ...
    switch (msg.type) {
      ...
      case "done":
        setStatus(
          "Done \u2014 " + msg.product_name +
          " | Serial: " + msg.serial +
          " | Campus: " + msg.school_name +
          " | Breaks: " + msg.break_count,
          "status-success"
        );
